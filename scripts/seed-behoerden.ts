...
const DEFAULT_SUPPLEMENT_PATH = path.resolve(
  process.cwd(),
  "data/behoerden-supplement.json",
);

// Additive research supplement (2026-07 Block 4): extra per-Bundesland
// Beglaubigungsstellen mapped to document-type slugs. Generated by
// scripts/gen-behoerden-supplement.py and committed under data/.
...
type Supplement = {
  doc_types: { slug: string; display: string }[];
  authorities: SupplementAuthority[];
};
const EMPTY_SUPPLEMENT: Supplement = { doc_types: [], authorities: [] };
...
export type SeedOpts = {
  ...
  sourcePath?: string;
  /**
   * Path to the additive research supplement JSON. Pass `null` to disable.
   * Defaults to the real file only for a full run (no `cachePath` override) so
   * tests stay isolated from the committed supplement.
   */
...
export async function seedBehoerden(opts: SeedOpts = {}): Promise<SeedResult> {
  ...
  // -------- 2b. LOAD SUPPLEMENT (additive research authorities) --------
  // Load the real supplement only for a full run; a `cachePath` override marks
  // a test, which stays isolated unless it passes `supplementPath` explicitly.
  const supplementPath =
    opts.supplementPath !== undefined
      ? opts.supplementPath
      : opts.cachePath
        ? null
        : DEFAULT_SUPPLEMENT_PATH;
  const supplement: Supplement =
    supplementPath && fs.existsSync(supplementPath)
      ? (JSON.parse(fs.readFileSync(supplementPath, "utf8")) as Supplement)
      : EMPTY_SUPPLEMENT;
  if (supplement.authorities.length > 0) {
    console.log(
      `[seed-behoerden] supplement: ${supplement.authorities.length} authorities, ` +
        `${supplement.doc_types.length} doc types (${path.basename(
          supplementPath as string,
        )})`,
    );
  }
  ...
  db.transaction((tx) => {
    ...
    // Supplement may introduce new document-type slugs (e.g. staatsexamen,
    // schulzeugnis) — ensure they exist before inserting supplement authorities.
    for (const dt of supplement.doc_types) {
      if (!docTypeMap.has(dt.slug)) {
        docTypeMap.set(dt.slug, { id: dt.slug, displayName: dt.display });
      }
    }
    ...
    // Additive research supplement (2026-07 Block 4): extra Beglaubigungsstellen
    // per Bundesland, mapped to document-type slugs. regierungsbezirkId is null.
    // States + document types already exist from the loops above.
    for (const a of supplement.authorities) {
      tx.insert(behoerdenAuthority)
        .values({
          id: randomUUID(),
          stateId: a.state_slug,
          ...
          officeHours: a.office_hours ?? null,
          notes: a.notes ?? null,
          specialRules: null,
