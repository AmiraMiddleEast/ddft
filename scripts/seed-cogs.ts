...
async function main(): Promise<void> {
  ...
  // 3) Fill-only overlay from data/cogs-supplement.json (2026-07 research,
  //    generated by scripts/gen-cogs-supplement.py). Fills ONLY currently-empty
  //    fields on matching ids and marks them verified (datenVollstaendig=true).
  //    Never overwrites existing non-empty data. Optional — skipped if absent.
  const overlayPath = path.join(cwd, "data/cogs-supplement.json");
  try {
    const overlayRaw = await readFile(overlayPath, "utf8");
    const overlay =
      (JSON.parse(overlayRaw) as { overlay?: Record<string, string | null>[] })
        .overlay ?? [];
    let filled = 0;
    for (const ov of overlay) {
      const id = ov.id;
      if (!id) continue;
      const row = merged.get(id) as Record<string, unknown> | undefined;
      if (!row) continue;
      let touched = false;
      ...
      if (row.datenVollstaendig !== true) {
        row.datenVollstaendig = true; // research-verified
        touched = true;
      }
      if (touched) filled++;
    }
    console.log(
      `[seed-cogs] overlay: filled/verified ${filled} rows (${path.basename(overlayPath)})`,
    );
  } catch {
    // No supplement file — skip.
  }
